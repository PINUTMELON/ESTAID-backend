...
--
-- 테이블 구성:
--   1. characters - 사용자가 등록한 원작 캐릭터 정보
--   2. plots      - AI가 생성한 스토리 씬 목록
--   3. images     - 씬별 AI 생성 이미지 (첫/마지막 프레임)
--   4. videos     - 씬별 AI 생성 영상 및 최종 병합 영상
--
...
-- =====================================================================
-- 1. characters 테이블
-- 설명: 사용자가 등록한 원작 캐릭터의 기본 정보를 저장한다.
...
-- 설명: characters 테이블에 UPDATE 발생 시 update_updated_at() 함수를 호출한다.
--       IF NOT EXISTS가 없으므로 DROP 후 재생성한다.
-- -------------------------------------------------------
...
--       씬 목록(scenes_json)은 JSON 문자열로 직렬화하여 단일 컬럼에 저장한다.
--       (추후 씬 수가 많아지면 Scene 테이블로 분리 예정)
-- =====================================================================
...
--       생성된 이미지는 영상 생성(videos) 시 시작/끝 프레임으로 사용된다.
-- =====================================================================
...
--       SCENE 타입: 씬 단위 영상 (3~5초, first/last 이미지로 생성)
--       MERGED 타입: 씬 영상들을 이어 붙인 최종 영상 (15~30초)
-- =====================================================================
